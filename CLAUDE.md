...
> 책: **"12-Factor Agents로 배우는 하네스 엔지니어링"** (에이전트/LLM 앱 개발자 대상) → 위키독스(WikiDocs)에 발행.
> 메타: **이 레포 자체가 12-Factor 원칙으로 만든 하네스 예시**다. 4부 케이스 스터디에서 스스로를 해부한다.
...
진행 상태는 **각 원고의 front-matter에만** 저장된다(머릿속·외부 금지). 세션이 끊겨도 위 두 명령으로 즉시 복원된다. ← 이게 Factor 5·6·12의 실물이다.
...
| factor 정의·출처·명칭 등 반복 사실 | `docs/verified-facts.md` (SoT) |
...
docs/
  ...
  verified-facts.md  ← 사실 SoT(factor 정의) + denylist
  publishing-runbook.md
  adr/               ← 결정 기록 (0001 하네스·SoT, 0002 교차검증·이미지)
