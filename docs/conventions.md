...
### (a) 다이어그램 — 코드가 진실의 원천
- 개념 흐름도(에이전트 루프, 상태 통합, reducer 등)는 **AI 래스터 이미지로 생성하지 않는다**. **SVG를 코드로 작성**한다(재현·diff 가능). (codex `imagegen` 같은 래스터 생성은 표지 아트엔 OK지만, 구조적 다이어그램엔 한글·화살표가 뭉개지고 비결정적이라 금지.)
- 소스: `assets/diagrams/<slug>/NN-이름.svg` (커밋 대상). 생성은 codex(독립 모델)가 SVG 코드로.
- 렌더: `python3 scripts/render_images.py` → `assets/screenshots/<slug>/NN-이름.png`(@2x). SVG 수정 시 재렌더.
- **겹침 회피 = 비전 교정 루프(필수).** LLM은 텍스트 폭을 못 재고 픽셀을 눈 감고 찍어 화살표·텍스트·박스 테두리가 겹친다. 그래서 생성 후 **`codex exec -i <렌더.png>`로 codex가 렌더 결과를 *보고* 좌표를 고치게** 한다(렌더→비전 진단→좌표 수정 SVG→재렌더). 내용·텍스트·스타일은 보존하고 위치·크기·라우팅만 손본다. SVG/결정론을 유지하므로 (a)의 원칙과 충돌하지 않는다.
- 렌더 후 **PNG를 Read로 눈 검수**(한글 글리프·잘림·겹침). 겹침이 남으면 비전 교정을 다시 돌린다.
