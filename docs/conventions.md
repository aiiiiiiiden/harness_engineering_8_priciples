...
- **금지**: 출처 없는 단정, "그냥 이렇게 하면 됩니다"식 생략, 검증 안 된 주장. 모든 원칙 설명은 원문(`docs/verified-facts.md`)과 일치.
...
- **핵심 개념**: 해당 factor/패턴을 한 줄 정의 + 비유 + 원문 인용으로 푼다.
...
- **8원칙은 이 책이 원문에서 도출한 커리큘럼**이다. 원문(OpenAI)은 원칙을 번호로 명시하지 않으므로, 본문은 "원문이 정한 8원칙"이 아니라 **"원문에서 도출한 8원칙"** 으로 제시하고 각 원칙을 원문 절에 접지한다.
...
### (a) 다이어그램 — 2트랙: SVG 소스 + ChatGPT 한국어 발행본 (ADR 0004)
> **콘텐츠 소스는 SVG, 발행 본문 이미지는 ChatGPT 한국어 래스터.** 한국어 라벨이 많은 다이어그램에서 SVG 렌더는 비전 교정을 거쳐도 한글 타이포그래피·여백 품질이 발행 수준에 못 미쳤다. 그래서 *내용*은 SVG로 설계·버전관리하되, *본문에 싣는 이미지*는 ChatGPT로 생성한 한국어 래스터를 채택한다. 이는 결정론 원칙(ch14)의 **의도적·문서화된 예외**이며 4부 자기채점에 감점으로 반영한다.

**(a-1) 콘텐츠 소스 — SVG (보존)**
- 개념 흐름도의 *구조·라벨·의미색*은 **SVG로 작성**한다(재현·diff 가능). 소스: `assets/diagrams/<slug>/NN-이름.svg`(커밋·보존 대상, 삭제 금지). 생성은 codex(독립 모델)가 SVG 코드로.
- 렌더 점검: `python3 scripts/render_images.py` → `assets/screenshots/<slug>/NN-이름.png`(@2x). 레이아웃·내용 설계의 참조용. (겹침이 보이면 `codex exec -i <렌더.png>` 비전 교정으로 좌표만 손봄.)
- SVG는 발행본의 **콘텐츠 명세(spec)** 역할 — ChatGPT 래스터를 생성·검수할 때 "무엇을 그려야 하는가"의 기준.

**(a-2) 발행 본문 이미지 — ChatGPT 한국어 래스터 (`![]()`가 가리키는 것)**
- 본문 `![캡션](...)`은 `assets/diagrams/chatgpt-renders/<slug>.ko.png`를 가리킨다(한국어판). 영어판 `.en.png`은 보관용.
- 생성 프롬프트는 `assets/diagrams/CHATGPT_IMAGE_PROMPTS.md`(표지-조화 스타일 + 장면)를 단일 원천으로 쓴다. 스타일·팔레트·용어는 SVG 소스 및 `verified-facts.md`와 일치시킨다.
- **결정론으로 재현되지 않으므로 저자가 직접 눈 검수**(필수): ① 한글 글리프·오타·잘림, ② 라벨 용어가 `verified-facts.md`와 일치(예: **병합**(머지 X)·**황금 원칙**(골든 X)·**진실의 원천**), ③ 캡션과 그림 내용 일치. 어긋나면 프롬프트의 해당 용어를 고쳐 재생성.
