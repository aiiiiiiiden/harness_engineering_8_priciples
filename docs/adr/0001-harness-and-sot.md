...
> 메타: 이 책이 가르치는 하네스 엔지니어링 원칙(특히 3·4 — 리포지터리가 기록 시스템·진실의 원천)을 **이 하네스가 그대로 실천**한다. 그래서 이 ADR의 결정은 곧 책의 예제이기도 하다. (이 책의 주제 전환 경위는 ADR 0003 참조.)
...
4. **진행 상태 = 각 원고 front-matter** (`status`, `wikidocs_page_id`). 별도 매니페스트를 저장하지 않아 drift를 원천 차단. 상태는 `status.py`가 front-matter를 스캔해 생성(저장 안 함) — **원칙 3·4(리포지터리가 기록 시스템·진실의 원천)의 실물**.
