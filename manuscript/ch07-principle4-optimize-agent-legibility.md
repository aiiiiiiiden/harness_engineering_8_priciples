...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| 낮은가독성  | 팀 Slack과 시니어 머릿속에만 있다                                                                                                                                              | DELETE 핸들러에서 물리 삭제를 짠다(그게 기본값이니까) → 사람이 리뷰에서 잡지 못하면 데이터가 사라진다 |
| 가독성 높음 | `docs/design-docs/soft-delete.md`에 결정+이유 기록, `src`의 삭제 경로가 `deleted_at`만 쓰도록 구조적 테스트(원칙 5와 협력), 스키마(`schema.sql`)에 `deleted_at` 컬럼 버전 관리 | 코드·스키마·문서만 읽고도 규칙을 추론한다                                                             |
