...
이 부록은 더 깊이 파고들 때 펴는 **1차 출처 지도**입니다. 이 책의 모든 주장이 어디에 접지하는지, 실습 도구의 공식 문서는 어디인지, 그리고 각 장이 정본의 어느 절에서 나왔는지를 한곳에 모았습니다. 2차 요약(블로그·영상)이 아니라 **원문**으로 곧장 가는 길을 알려드립니다.

## 핵심 개념

자료를 세 층으로 나눕니다. **1차 출처(정본)**가 가장 위, 그 아래 **실습 도구 공식 문서**, 맨 아래 **이 책의 내부 기준(SoT)**입니다. 주장을 확인할 때는 항상 위층부터 봅니다.

### (가) 1차 출처 — 정본

| 자료 | 무엇 | 링크 |
|---|---|---|
| **Harness Engineering** (OpenAI, Ryan Lopopolo, 2026-02-11) | 이 책의 정본. "사람이 조종하고, 에이전트가 실행한다"와 8원칙의 도출 근거. 빈 리포에서 5개월간 약 100만 줄을 Codex만으로 구축한 실험 보고. | https://openai.com/index/harness-engineering/ |

> 이 책의 8원칙은 위 글의 **11개 절 중 원칙을 담은 8개 절에서 도출**한 것입니다(정본은 원칙을 번호로 매기지 않습니다). 절 ↔ 원칙 대응은 아래 (라)를 보세요.
...
| **OpenAI Codex** | 정본 실험의 코딩 에이전트. 정본 글과 같은 출처에서 다룬다. | https://openai.com/index/harness-engineering/ |

> 외부 라이브러리 문서는 정본의 방식대로 **요약본을 리포에 둡니다**. `docs/references/*-llms.txt`에 핵심만 추려 버전 관리하면, 에이전트가 매번 외부를 긁지 않고도 안정적으로 참조합니다(원칙 3, ch06).
...
| 파일 | 역할 |
|---|---|
| `docs/verified-facts.md` | 8원칙 정의·출처·명칭의 진실의 원천. 정본 절 매핑과 denylist 포함. |
...
각 장이 정본의 어느 절에 접지하는지입니다. 정본 절 제목은 원문(영문) 그대로 옮겼습니다.
...
이건 원칙 3(기록 시스템)을 책이라는 산출물에 적용한 것입니다. 정본은 "에이전트가 검색할 수 없는 지식은 없는 것과 같다"고 했는데, 독자에게도 똑같습니다. 본문이 "정본에 따르면…"이라고만 하고 어느 절인지 가리키지 않으면, 그 주장은 검증할 수 없는 채로 남습니다.
...
- **2차 출처로 때우기.** 정본 대신 그것을 요약한 블로그·영상을 인용합니다. 요약은 틀리거나 낡기 쉽습니다. 항상 원문을 먼저 가리키고, 2차 자료는 보조로만 둡니다.
...
- **버전 없는 인용.** "정본에 따르면"으로 끝내고 어느 절인지 안 밝히는 것. 독자가 확인할 수 없습니다. 위 (라)처럼 절 단위로 접지합니다.

## 핵심 정리 / 체크리스트

- [ ] 주장은 **1차 출처**(정본·공식 문서)로 추적 가능해야 한다. 2차 요약은 보조.
- [ ] 8원칙의 근거는 정본 11개 절 중 8개. 절 ↔ 원칙 대응은 위 (라)를 따른다.
