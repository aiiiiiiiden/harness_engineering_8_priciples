...
참고 자료를 정리하는 건 멋을 부리려는 게 아니라, 주장을 떠받치는 **신뢰의 사슬**을 만드는 일입니다. 기술서라면 어떤 주장이든 그 근거를 되짚어갈 수 있어야 합니다. 독자가 "정말 그런가?"를 1차 출처에서 직접 확인할 수 있어야 하고, 그러려면 출처가 2차 요약이 아니라 원문을 가리켜야 합니다.
...
규모가 커지면 이 문제는 더 또렷해집니다. 장이 스무 개로 늘면 출처도 흩어집니다. 링크를 한곳(`toc.json`)에 등록하고 기계로 점검하지 않으면, 죽은 링크와 어긋난 인용이 조용히 쌓여 엔트로피(원칙 8)가 됩니다.
...
- **2차 출처로 때우기.** 원문 대신, 그걸 요약한 블로그나 영상을 인용하는 경우입니다. 요약은 틀리거나 낡기 쉽습니다. 항상 원문을 먼저 가리키고, 2차 자료는 보조로만 둡니다.
