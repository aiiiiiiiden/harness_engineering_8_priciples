...
![하네스 성숙도 사다리 다이어그램: L1 없음(빈 레포)→L2 문서화(규약은 있으나 강제 없음)→L3 구조화(SoT·맵·스크립트, 수동 점검)→L4 강제(게이트가 코드로 차단)→L5 자율(게이트가 루프에 인코딩, 모델 다양성·엔트로피 자동 GC)로 오르는 계단. 문서화 위에 구조, 구조 위에 기계 강제, 그 위에 자율 루프가 쌓인다.](../assets/screenshots/ch17-self-assessment-scorecard/01-maturity-ladder.png)
