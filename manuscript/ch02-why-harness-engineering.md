...
> **정본은 원칙을 번호로 세지 않습니다.** OpenAI의 글은 서술형이라 "N개 원칙"이라는 목록을 제시하지 않습니다. 이 책의 8원칙은 정본의 목차 절(엔지니어 역할 재정의·가독성 향상·기록 시스템·기계적 강제·병합 철학·자율성·가비지 컬렉션 등)을 **한 절당 한 원칙으로 도출**한 *이 책의 커리큘럼*입니다. 그래서 본문은 "정본이 정한 8원칙"이 아니라 **"정본에서 도출한 8원칙"** 으로 부릅니다. 각 원칙은 2부에서 정본의 해당 절을 직접 인용하며 접지합니다.
