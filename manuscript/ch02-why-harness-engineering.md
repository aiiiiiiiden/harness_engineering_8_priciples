...
![같은 증상(에이전트가 매번 인증 로직을 제멋대로 다시 구현)에 대한 두 반응 대비. 왼쪽 '프롬프트로 때우기'는 이번 호출만 반짝 통과하고 다음 세션에 같은 실수가 재발한다. 오른쪽 '환경을 고치기'는 결정을 docs에 기록(원칙 3)하고 구조적 테스트로 막아(원칙 5) 모든 미래 실행에 영구 적용된다.](../assets/screenshots/ch02-why-harness-engineering/01-prompt-vs-environment.png)
