...
계층 강제도 같은 방식입니다. 가상의 예제 프로젝트인 `todo-api`에 "UI 계층은 Repo 계층을 직접 부르지 않는다"는 규칙을 구조적 테스트로 처리하면, 에이전트가 지름길을 만들어도 빌드가 실패하도록 막아줍니다. **무엇을** 지킬지(계층 방향)는 고정하되, Service를 어떻게 구현할지는 에이전트에게 맡깁니다. 규칙을 강제하지만, 규칙 이외는 에이전트에게 자율을 보장합니다.
