...
이 패턴이 필요한 이유는 **불투명한 산출물**에서 비롯됩니다. 어떤 산출물이 소스 없이 한 번 만들어진 블롭(AI가 생성한 래스터 이미지, 손으로 적어 넣은 값, 손편집한 생성 파일)이면, 셋이 동시에 깨집니다. 무엇이 바뀌었는지 diff할 수 없고, 같은 결과를 다시 만들 수 없으며, 에이전트가 그걸 읽거나 고칠 수 없습니다. 결정론적 산출물은 이 셋을 한꺼번에 해결합니다. 소스가 텍스트(코드)라 diff·리뷰가 되고, 생성이 재현 가능하니 누구나 같은 결과를 얻고, 소스를 고치면 산출물이 따라옵니다.
...
이 패턴은 책을 만드는 일 이외의 케이스에서도 유용합니다. DB 타입은 스키마에서 생성하고, API 클라이언트는 OpenAPI 정의에서 생성하고, 레퍼런스 문서는 코드 주석과 스키마에서 생성합니다. 손으로 동기화하지 말고 생성해야 합니다. 그리고 빌드를 결정론적으로 만들면(같은 입력 → 같은 출력) 캐시, 검증, 재현이 공짜로 따라옵니다. 무엇이든, 산출물은 소스의 파생이고 소스가 진실입니다.
