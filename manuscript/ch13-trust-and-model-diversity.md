...
> **자기참조 — 이 책이 이 게이트로 만들어집니다.** 이 책의 사실은 집필 모델(Claude) 하나가 아니라, *독립 모델*(codex)이 한 번 더 교차검증해야 reviewed가 됩니다. 집필과 검증이 다른 모델이라, 한 모델의 사각지대(원칙 순서 혼동·출처 오귀속·원문이 명시 안 한 카운트 단정)를 다른 모델이 잡습니다. 두 모델 + 기계 검증이 *합의*해야 비로소 통과합니다. (이 게이트 파이프라인의 전체 구성·도구는 4부 케이스 스터디에서 해부합니다.)
