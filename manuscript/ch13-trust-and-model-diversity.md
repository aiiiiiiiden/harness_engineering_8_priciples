...
![산출물이 게이트1 기계 검사(스키마·규칙·링크, 결정론적, 원칙 5)→게이트2 의미 검사(같은 모델 심층 검토)→게이트3 독립 검사(다른 모델이 사실 교차검증, 모델 다양성)를 직렬로 통과하는 그림. 셋이 합의해야 통과하며, 한 게이트라도 실패하면 거기서 막힌다.](../assets/screenshots/ch13-trust-and-model-diversity/01-verification-gates.png)
