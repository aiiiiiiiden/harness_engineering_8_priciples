...
![한 프롬프트가 닫힌 루프를 돈다: 상태 검증 → 버그 재현 → 수정 구현 → 앱 실행·검증(원칙 2) → PR 열기 → 피드백 응답 → 빌드 실패 감지·수정. 루프 안에서 판단이 필요할 때만 사람에게 에스컬레이션(원칙 1)하고, 통과하면 병합한다. 검증·복구 단계가 인코딩돼 있어야 루프가 끊기지 않는다.](../assets/screenshots/ch10-principle7-encode-autonomy-loop/01-autonomy-loop.png)
