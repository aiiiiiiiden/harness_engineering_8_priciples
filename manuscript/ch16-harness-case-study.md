...
- **모델의 사각지대 때문에 오류를 놓친다.** 콘텐츠를 생성한 모델이 스스로 검토하면 자기 사각지대에 갇혀 오류를 놓칩니다(13장). 이 프로젝트 저장소는 `crosscheck.py`로 집필에 사용한 모델과 다른 모델을 사용합니다.
...
- **스스로 검증하는 것을 교차검증이라 부르기.** 컨텐츠를 작성하는 모델에게 "검증해줘"라고만 하면 사각지대에 가려 문제를 놓칩니다(13장). 벤더가 다른 모델(codex)을 사용해 검증해야 사각지대에 의해 발생하는 문제를 줄일 수 있습니다.
