...
이 장을 끝내면 **이 책 집필에 사용한 프로젝트 저장소를 열어 앞에서 배운 8원칙과 4가지 패턴이 하네스로 어떻게 구축되어 있는지** 알게 됩니다. 1~3부에서 올바른 하네스 구축을 위한 원칙과 패턴을 소개했다면, 4부는 그 하네스가 실제로 작동한다는 증거입니다. 추상적이던 하네스가 실제 코드로 어떻게 구현됐는지 한 파일씩 살펴보겠습니다. 지금까지 본문은 이 책 집필에 사용한 프로젝트의 하네스를 의도적으로 가렸지만, 케이스 스터디인 4부에서는 그 가림막을 걷고 안을 해부하겠습니다.

## 핵심 개념

이 프로젝트 저장소는 한 줄로 요약하면 **코드가 진실의 원천(SoT)이 되어 "책 원고"를 산출물로 뽑아내는 하네스입니다.** 빈 디렉토리에서 시작해, 에이전트가 안정적으로 한 장씩 써 내려가도록 계획, 작성, 검증, 상태 기록을 하네스로 구축했습니다. 이 책의 프로젝트 저장소는 아래 주소에서 볼 수 있습니다.

https://github.com/aiiiiiiiden/harness_engineering_8_priciples

이 장을 읽으며 실제 파일을 함께 열어 보면 좋습니다. 이 책의 하네스를 워크플로우로 그리면 다음과 같습니다.
...
"아는 것"과 "쓰는 것" 사이에는 큰 간극이 있습니다. 이 프로젝트 저장소의 하네스 없이 그냥 LLM을 사용해 "이런 책 한 권 써줘"라고 했다면 무엇이 무너지는지가 이 장의 동기입니다.

- **사실이 장마다 어긋난다.** 원칙 번호, 지식, 출처를 매번 모델에 의존하면, [5장](https://wikidocs.net/363223)과 [12장](https://wikidocs.net/363231)이 다른 정의를 쓸 수 있습니다. 이 프로젝트는 사실을 `docs/verified-facts.md` 한 곳에 두어 진실의 원천이 되게 하고, 이를 `verify.py`를 사용해 기계적으로 검증합니다.
- **진행 상태를 잃어버린다.** 어디까지 썼는지를 사람의 기억이나 외부 메모에만 남겨 두면 세션이 끊길 때 사라집니다. 이 프로젝트 저장소에서는 각 챕터의 상태를 각 원고의 front-matter에 두고 `status.py`가 스캔해 마지막 상태를 복원합니다(원칙 3).
- **모델의 사각지대 때문에 오류를 놓친다.** 콘텐츠를 생성한 모델이 스스로 검토하면 자기 사각지대에 갇혀 오류를 놓칩니다([13장](https://wikidocs.net/363232)). 이 프로젝트 저장소는 `crosscheck.py`로 집필에 사용한 모델과 다른 모델을 사용합니다.
- **AI Slop이 그대로 발행된다.** 사실이 맞아도 번역체·기계적 리듬이 남으면 한글 기술서로서 신뢰를 잃습니다. 이 프로젝트 저장소는 humanize 스킬을 사용해 내용은 변경하지 않고 문체만 자연스럽게 변경합니다.

요컨대 하네스가 없으면 그럴듯한 초안은 나오지만, **사실이 어긋나고, 진행 상태가 사라지고, 사각지대가 오류를 부르고, AI 티가 그대로 남는** 문제가 시간이 갈수록 커집니다. 이 프로젝트 저장소는 그 넷을 각각 코드로 막습니다.
...
**`scripts/crosscheck.py` — 모델 다양성 (원칙 1·ch13).** 집필은 Claude가 맡고, 사실 검증은 벤더가 다른 모델인 Codex가 따로 합니다. 신뢰성 장치는 세 가지입니다. 특히 JSON 스키마를 강제해(`--output-schema`) 구조화된 결과를 보장하고, read-only 샌드박스라 Codex가 원고를 수정하지 못하며, 차단할 수준의 오류가 없으면 `crosscheck: pass`를 front-matter에 업데이트하도록 `verify.py`의 게이트와 맞물려 동작합니다. pass 도장을 손으로 변경하지 않는다는 게 핵심입니다. 검증을 실제로 통과해야만 pass로 갱신됩니다.
...
- **게이트를 사람 규율에 맡기기.** "글을 발행하기 전 체크리스트를 만들고 사람이 검증하기"와 같은 규칙은 손쉽게 어기게 됩니다. `verify.py`가 도장 없으면 차단하듯(원칙 5), 기계적으로 검증할 수 있는 게이트를 두어야 합니다.
- **pass 도장을 직접 수정하기.** `crosscheck: pass`를 검증 없이 직접 작성하면 게이트는 명목만 남습니다. 검증에 사용된 스크립트가 실제 통과 시에만 자동으로 갱신하도록 해야 합니다.
- **스스로 검증하는 것을 교차검증이라 부르기.** 컨텐츠를 작성하는 모델에게 "검증해줘"라고만 하면 사각지대에 가려 문제를 놓칩니다([13장](https://wikidocs.net/363232)). 벤더가 다른 모델(codex)을 사용해 검증해야 사각지대에 의해 발생하는 문제를 줄일 수 있습니다.
...
- [ ] `verify.py`가 기계적인 검증을 강제하고, `reviewed`는 **교차검증·윤문 검증을 마쳐야** 통과합니다(원칙 5).
