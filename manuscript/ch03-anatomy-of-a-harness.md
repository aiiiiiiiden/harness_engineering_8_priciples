...
![하네스를 다섯 부품으로 해부한 그림 — 진입점·지도(원칙 3), 기록 시스템(원칙 3·4), 기계적 강제(원칙 5), 관측성(원칙 2), 피드백 루프(원칙 1·7·8). 원칙 6은 부품을 가로지르는 운영 정책(병합)으로 따로 둔다. 아래 띠는 빈 레포(L1)에서 다섯 부품이 맞물린 자율(L5)로 오르는 성숙도를 나타낸다.](../assets/diagrams/chatgpt-renders/ch03-anatomy-of-a-harness.ko.png)
