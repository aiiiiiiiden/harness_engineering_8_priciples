...
| **기록 시스템** | 결정, 계획, 스키마를 리포지터리에 버전 관리   | 3/4원칙 (기록 시스템·에이전트 가독성) |
...
다섯 부품은 추상적인 개념이 아니라 리포지터리 안에 위치합니다. 이 책의 샘플 프로젝트를 할 일 관리 API 서버라고 가정하고 이를 빈 레포에서 키운다고 할 때, 리포지터리에 부품 이름을 달면 아래와 같습니다.
...
- **codex 같은 독립 모델 CLI** — 검증 부품. 한 모델이 만든 것을 다른 모델이 교차검증하는 모델 다양성 실습에 사용됩니다.
