...
**하네스란, 코딩 에이전트가 안정적으로 일할 수 있도록 설계된 환경 전체다.** 에이전트가 읽고 의지하는 기록 시스템(문서·스키마·계획), 실수를 빌드 단계에서 막는 기계적 불변식(린터·구조적 테스트), 에이전트가 자기 작업을 직접 검증하게 해주는 관측성, 그리고 실패를 스스로 고치게 하는 피드백 루프까지. 모델이 토큰을 뱉는 그 한 번의 호출은 에이전트가 사용하는 모델의 몫입니다. 하지만 그 모델이 기대대로 동작하게 만드는 일은 하네스가 떠맡습니다.
